"""
소방 합격수기 태그 추출 (extract_tags.py 의 소방판)
- 대상 DB: fire_reviews.db (공무원 reviews.db 와 분리)
...
LOG_FILE = "extract_tags_fire.log"
NORMALIZE_EVERY = 100
...
def extract_tags(item, existing_tags):
    ...
    prompt = f"""다음 소방공무원 합격수기를 읽고 JSON만 반환해줘. 설명 없이 JSON만.

[규칙]
1. tags: 이 사람을 설명하는 키워드 자유롭게 추출
   - 생활상황/나이대/수험이력/공부법/루틴/멘탈관리/체력준비 등 모든 특징
   - 개수 제한 없음
...
2. instructors: 언급된 강사
   - 성+이름 풀네임 (쌤/T/선생님 제거)
   - 과목: 소방학개론/소방관계법규/행정법/국어/영어/한국사/소방영어/소방한국사/면접 등
